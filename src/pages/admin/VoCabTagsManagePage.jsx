...
export function VoCabTagsManagePage() {
  ...
  const handleSave = async (form) => {
    try {
      ...
      console.log('Lỗi khi lưu danh mục! ', error.message )
    } finally {
      setIsSaving(false)
    }
  }
...
return (
    <div>
      <PageHeader title="Các thẻ từ vựng" action="Thêm danh mục" onAction={() => setModal({})} />
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden relative min-h-[300px]">
        ...
        <table className="w-full">
          <thead className="bg-gray-50 text-xs font-bold text-gray-500 uppercase tracking-wider">
            <tr>
              {['STT', 'Slug', 'Tên thẻ', 'Màu', 'Thao tác'].map(h => (
              ...
              <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <div className="flex gap-2">
                    <button onClick={() => setModal(c)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-50 text-sm transition-all">✏️</button>
                    <button onClick={() => setDeleteId(c.id)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-red-100 hover:bg-red-50 text-sm transition-all">🗑️</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <ConfirmDialog open={!!deleteId} onClose={() => !isDeleting && setDeleteId(null)} onConfirm={handleDelete} title="Xác nhận xóa" description="Bạn có chắc chắn muốn xóa danh mục này không? Hành động này không thể hoàn tác." isLoading={isDeleting} />
    </div>
  )
}